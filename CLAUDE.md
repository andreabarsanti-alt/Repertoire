...
- **CRD reader**: A song can carry a `crd` field — a plain-text chord/lyric sheet, entered manually (Add Song modal, inline edit, or the full-page editor). When a song has `crd`, its title everywhere (songs table, setlists, play mode, visualizer pills) opens the in-app CRD reader (`openCrdReader`) instead of the UG link. Because it's an in-page overlay rather than a navigation, it also sidesteps the AirTurn/UG Universal-Link app-switch issue during Play mode.
  ...
  - **Transpose** (`crdTranspose`, −11..+11): −/+/reset controls, plus **Store transposed** (`crdStoreTransposed`) which rewrites the saved `crd` text at the current transpose and resets the control.
  ...
  - **Editable Key/Capo**: the Key and Capo badges in the reader header are inputs (`crd-key-input`/`crd-capo-input`) — typing into them only updates `crdEditKey`/`crdEditCapo` (`crdMetaEdited`), it doesn't touch the song or the displayed transpose. Clicking **Store transposed** commits whichever of the three (transpose, Key, Capo) actually changed in one go, then re-baselines `crdBaseTranspose` from the (possibly just-changed) Capo — same as reopening the reader. `crdIsDirty()`/`crdUpdateStoreBtn()` keep the button's enabled state in sync with all three, not just the transpose delta.
