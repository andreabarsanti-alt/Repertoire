...
- **CRD reader**: A song can carry a `crd` field — a plain-text chord/lyric sheet, entered manually (Add Song modal, inline edit, or the full-page editor). When a song has `crd`, its title everywhere (songs table, setlists, play mode, visualizer pills) opens the in-app CRD reader (`openCrdReader`) instead of the UG link. Because it's an in-page overlay rather than a navigation, it also sidesteps the AirTurn/UG Universal-Link app-switch issue during Play mode.
  ...
  - **Re-root** (`crd-reroot-chk` → `crdReRoot`/`crdReRootDelta`, toggled by `crdReRootToggle`): for fixing a sheet that was transcribed a few semitones off from its labelled key — deliberately bypasses all capo math. Checking it shows the *raw* saved `crd` (Playing key/Capo hidden; only Key is editable) and repoints −/+/↺ (`crdTKeyStep`/`crdTKeyReset`) at a plain semitone delta instead of the playing key. Unchecking bakes that delta into `s.crd` (`transposeCrdText`) and writes the new `s.key`, and **resets `s.capo`/`crdEditTKey` to match the new key** (`computeCapo(newKey, newKey)`, i.e. 0 unless half-step tuning adds its usual +1) — the sheet now literally *is* the declared key, so "played as" resets to that key rather than carrying forward whatever capo/playing-key was in effect before. A no-op toggle (nothing changed) exits silently; declining the confirm() or typing an unparsable key re-checks the box via the next `renderCrdBody()` rather than touching the checkbox DOM directly.
  ...
  - **Full-page editor** (`#crd-editor`, `openCrdEditor`/`closeCrdEditor`): the "⤢ Full editor" button next to the CRD field (Add Song modal `f-crd`, Songs table inline edit `ie-crd`, or setlist inline edit `sl-ie-crd`) pops a full-screen split view — the raw textarea plus a live-rendered preview (`renderCrdEditorPreview`, same `renderCrdHTML` the reader uses) side by side, updating on every keystroke. It also carries its own mini Key/Playing-key/Capo row (`crdEditorKey`/`crdEditorTKey`, mirroring the reader's), so you can dial in the sheet's key and playing key while still writing it. It works even for a not-yet-saved song (no `db.songs` entry to read) by deriving the sibling Key/Capo field ids straight from `targetId` (`"f-crd"` → `"f-key"`/`"f-capo"`, etc.) — there's always a `-crd`/`-key`/`-capo` triplet in the calling form. On "← Done", the crd text, Key, *and* the resulting Capo (`computeCapo(crdEditorKey, crdEditorTKey)`) all sync back into those sibling fields (`dispatchEvent('input')`, so the outer form/Save picks them up) — same Key+Playing-key→Capo relationship as the reader, just landing directly in the song's fields instead of a separate Save-capo step.
