...
- **CRD reader**: A song can carry a `crd` field — a plain-text chord/lyric sheet, entered manually (Add Song modal, inline edit, or the full-page editor). When a song has `crd`, its title everywhere (songs table, setlists, play mode, visualizer pills) opens the in-app CRD reader (`openCrdReader`) instead of the UG link. Because it's an in-page overlay rather than a navigation, it also sidesteps the AirTurn/UG Universal-Link app-switch issue during Play mode.
  ...
  - **Re-root** (`crd-reroot-chk` → `crdReRoot`/`crdReRootDelta`, toggled by `crdReRootToggle`): for fixing a sheet that was transcribed a few semitones off from its labelled key — deliberately bypasses all capo math. Checking it shows the *raw* saved `crd` (Playing key/Capo hidden; only Key is editable) and repoints −/+/↺ (`crdTKeyStep`/`crdTKeyReset`) at a plain semitone delta instead of the playing key. Unchecking bakes that delta into `s.crd` (`transposeCrdText`) and writes the new `s.key`, but **leaves `s.capo` untouched on purpose** — it seeds `crdEditTKey` to `key + delta` so the view doesn't jump, then "Save capo" (already dirty, since the untouched capo no longer matches `computeCapo(key, crdEditTKey)`) is there as a separate, explicit follow-up if capo should resync too. A no-op toggle (nothing changed) exits silently; declining the confirm() or typing an unparsable key re-checks the box via the next `renderCrdBody()` rather than touching the checkbox DOM directly.
